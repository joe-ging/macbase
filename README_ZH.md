...
#### **方法 A：经典快捷方式 (适用于旧版本 macOS)**
- 在应用程序文件夹中，**右键点击 (或 Control-点击)** `macbase` 图标，然后选择 **“打开”**。
- **注意：** 此方法在 macOS 15 (Sequoia) 及 16 (Tahoe) 中可能已失效。
...
#### **方法 C：Sequoia & Tahoe 终端修复 (推荐)**
如果系统设置中没有出现“仍要打开”按钮（Sequoia 的已知 Bug）或点击无效，请打开 **终端 (Terminal)** 并运行：
```bash
sudo xattr -cr /Applications/macbase.app
```
这会移除隔离标记并允许应用立即启动。

#### **方法 D：DMG 挂载修复 (如果 DMG 文件本身打不开)**
在 macOS Sequoia/Tahoe 中，有时 DMG 文件即使在开启了“任何来源”的情况下也无法挂载。请使用此命令强制挂载：
```bash
hdiutil attach -noverify ~/Downloads/macbase.dmg
```

#### **方法 E：降维打击 (若上述均无效)**
既然 macbase 是开源核心项目，您可以直接运行源码以 100% 绕过系统的二进制检查：
1. 克隆本仓库。
2. 参考下方的 **“开发环境配置”** 步骤运行。
3. **优势：** macOS 从不阻止 Python/Node.js 等解释型代码的运行。
